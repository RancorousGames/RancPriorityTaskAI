...
//*************************************************************************
	...
	/*  Last value calculated by CalculateUtility() */
...
//*************************************************************************
	...
	UFUNCTION(BlueprintCallable,BlueprintNativeEvent, Category = Basic, Meta = (SuccessToolTip = "Whether the task succeeded, this is only used for invoked tasks."))
    ...
    UFUNCTION(BlueprintCallable, Category = Basic, Meta = (MaxWaitTimeToolTip="Maximum time to wait in seconds. Use 0 for indefinite waiting.", OverrideInterruptionTypeToolTip="Set to true to override the default interruption type while waiting.", InterruptTypeWhileWaitingToolTip="The interruption type to use while waiting."))
    void BeginWaiting(double MaxWaitTime = 0.f, bool OverrideInterruptionType = false, ERAIInterruptionType InterruptTypeWhileWaiting = ERAIInterruptionType::Never);
	
	/*  Call this to indicate we are done waiting  */
	UFUNCTION(BlueprintCallable, Category = Basic, Meta = (ExpandEnumAsExecs = "ReturnBranch", InterruptTypeToReturnToToolTip="InterruptTypeToReturnTo is only used if OverrideInterruptionType was set to true when starting the wait."))
	...
	UFUNCTION(BlueprintPure, Category = ManagerInterface)
...
//*************************************************************************
	...
	const int MaxTaskLoopCount = 14; // Engine will detect after 15 repeats in the same frame
	const float LoopCountDetectionPeriod = 2.f; // After how many seconds do we reset loopcount
